...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    if not feral_hogs:
        ...
        h = say
        prescore0, prescore1,roll0, roll1 = 0, 0, 0, 0
        while score0 < goal and score1 < goal:
            if who == 0:
                roll0 = strategy0(score0, score1)
                if abs(roll0 - prescore0) == 2:
                    prescore0 = take_turn(roll0, score1, dice)
                    score0 += prescore0 + 3
                else:
                    prescore0 = take_turn(roll0, score1, dice)
                    score0 += prescore0
                ...
                if abs(roll1 - prescore1) == 2:
                    prescore1 = take_turn(roll1, score0, dice)
                    score1 += prescore1 + 3
                else:
                    prescore1 = take_turn(roll1, score0, dice)
                    score1 += prescore1
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 1:    
            pre, h = last_score, running_high
            gain = score1 - pre
            if h < gain:
                h = gain
                print(h, "point(s)! That's the biggest gain yet for Player 1")
            return announce_highest(who, score1, h)
        elif who == 0:
            pre, h = last_score, running_high
            gain = score0 - pre
            if h < gain:
            ...
            return announce_highest(who, score0, h)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    i, max = 1, 0
    while i <= 10:
        avr = make_averaged(roll_dice, trials_count)(i, dice)
        if max < avr:
            max = avr
            ans = i
        i += 1
