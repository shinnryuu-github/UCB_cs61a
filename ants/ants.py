...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        curr = self.place
        while curr != beehive:
            if lc >= self.min_range and lc <= self.max_range:
                if rANTdom_else_none(curr.bees) != None:
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.contained_ant:
            if isinstance(self.contained_ant, QueenAnt) and self.contain_ant.No != 0:
                self.contained_ant = None
            else:
    ...
    def action(self, gamestate):
        ...
        if self.No != 0:
            if self.is_contained:
                self.containner.contained_ant = None
                self.place = None
            else:
                self.armor = 0
                self.place.ant = None
            ...
            def powerup(place):
                if place.ant:
                    if isinstance(place.ant, ContainerAnt):
                        if place.ant.power:
                            place.ant.damage *= 2
                            place.ant.power = False
                        if place.ant.contained_ant:
                            if place.ant.contained_ant.power:
                                place.ant.contained_ant.damage *= 2
                                place.ant.contained_ant.power = False
                    else:
                        if place.ant.power:
                            place.ant.damage *= 2
                            place.ant.power = False
