...
def balanced(m):
    ...
    if is_planet(end(left(m))):
        total_left = size(end(left(m))) * length(left(m))
    else:
        total_left = total_weight(end(left(m))) * length(left(m))
    if is_planet(end(right(m))):
        total_right = size(end(right(m))) * length(right(m))
    else:
    ...
    if is_mobile(end(left(m))):
        if is_mobile(end(right(m))):
            return balanced(end(left(m))) and balanced(end(right(m))) and total_left == total_right
        return balanced(end(left(m))) and total_left == total_right
    if is_mobile(end(right(m))):
        return balanced(end(right(m))) and total_left == total_right
...
def totals_tree(m):
    ...
    if is_planet(m):
        return tree(size(m))
    return tree(total_weight(m), [totals_tree(arm_mobile) for arm_mobile in [end(left(m)), end(right(m))]])
...
def has_path(t, phrase):
    ...
    if is_leaf(t):
        return label(t) == phrase
    if label(t) == phrase[0]:
        return True and True in [has_path(branch, phrase[1:]) for branch in branches(t)]
    else:
        return False
